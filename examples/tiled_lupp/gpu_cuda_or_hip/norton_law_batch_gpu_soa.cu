...
/// in SoA layout and solved through the external residencies of the
...
///
/// Companion of norton_law_batch_gpu.cu: same law, same batch, opposite
/// placement of the solver operands. Here the jacobian, the pivot and
/// the right-hand side of every point are slices of device memory
/// batches, structure-of-arrays: element k of point t sits at
/// base[k * points + t]. The solver is called with the residency
/// booleans set to false and the batch stride, so consecutive threads
/// touch consecutive addresses and every access is coalesced.
///
...
/// solver change. The price is that the batch of systems occupies
/// device memory, which bounds its size.
...
/// history with the Newton systems in the SoA batches, records the
/// state before the last step for the host-side checks, and writes the
/// final stress, tangent operator and success flag.
...
__global__ void integrate_batch(const int points, const int steps, const double dt, double* J,
    ...
    for (int s = 0; s < steps && success; ++s) {
        ...
        success = integrate<false, false, false>(deto, dt, eel, p_t, sig_t, Dt_t, J + t, points,
                                                 piv + t, points, r + t, points);
    }
...
int main(int argc, char** argv) {
    ...
    const int block = 256;
    integrate_batch<<<(points + block - 1) / block, block>>>(
