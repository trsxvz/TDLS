...
/// fixed-size right-hand side and solution of different extents must be
...
/// and passes only when the compiler emits the extent diagnostic of the
/// adaptors: the two extents are provably inconsistent whatever the
/// runtime dimension of the matrix.
...
/// \brief Indexing policy of the fixed-size vector mock.
/// \tparam N extent
template<int N>
...
/// \brief Fixed-size contiguous vector mock.
/// \tparam N extent
template<int N>
struct MockVector {
    using indexing_policy = MockVectorPolicy<N>;
    double v[N];
    double* data() {
        return v;
    }
