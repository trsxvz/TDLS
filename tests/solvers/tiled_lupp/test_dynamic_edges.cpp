...
void anchor(const int n, const int count, const double tolerance, const std::uint64_t seed) {
    using Solver =
        tdls::TiledLUppSolverDynamic<double, tdls::TiledLUppConfig<double>{.tile_size = tile_size}>;
...
TDLS_TEST_CASE("tiledlupp/dynamic-edges/tile-grid-helpers") {
    ...
    TDLS_CHECK(S4::tile_size_at(0, 9) == 4);
    TDLS_CHECK(S4::tile_size_at(4, 9) == 4);
    TDLS_CHECK(S4::tile_size_at(8, 9) == 1);
    TDLS_CHECK(S4::tile_size_at(0, 2) == 2);
    using S6 = tdls::TiledLUppSolverDynamic<double, tdls::TiledLUppConfig<double>{.tile_size = 6}>;
    TDLS_CHECK(S6::num_tiles(13) == 3);
    TDLS_CHECK(S6::tile_size_at(12, 13) == 1);
}

TDLS_TEST_CASE("tiledlupp/dynamic-edges/smallest-full-tile-grid/n=2,tile_size=2") {
    anchor<2>(2, 300, 1e-9, 230100);
}

TDLS_TEST_CASE("tiledlupp/dynamic-edges/single-partial-tile/n=3,tile_size=8") {
    anchor<8>(3, 300, 1e-9, 230200);
}

TDLS_TEST_CASE("tiledlupp/dynamic-edges/large-dimension/n=200,tile_size=6") {
