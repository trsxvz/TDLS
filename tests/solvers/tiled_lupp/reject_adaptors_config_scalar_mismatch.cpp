/// \file
/// \brief Negative compilation test: a configuration value whose scalar
/// type differs from the matrix element type must be rejected by the
...
/// and passes only when the compiler emits the scalar-match diagnostic
/// of the adaptors (a float threshold set silently on a double solver
/// would change the pivoting policy).
...
constexpr auto config = tdls::TiledLUppConfig<float>{};
