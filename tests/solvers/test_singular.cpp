...
/// normal entries), the verdicts of the static solver (both schedules),
/// the dynamic solver and the reference must all agree.
...
void check_verdicts(const double* A0, const double* b0, const bool solvable) {
    using ConfigRL = tdls::TiledLuConfig<double, TS, tdls::TiledLuSchedule::RightLooking>;
    using ConfigLL = tdls::TiledLuConfig<double, TS, tdls::TiledLuSchedule::LeftLooking>;
    std::vector<double> A(N * N), x(N);
    std::vector<int> piv(N);

    std::copy(A0, A0 + N * N, A.begin());
    const bool ok_rl = tdls::TiledLuSolverStatic<double, N, ConfigRL>::solve<false, false, false>(
        A.data(), 1, piv.data(), 1, b0, x.data(), 1);
    TDLS_CHECK(ok_rl == solvable);

    std::copy(A0, A0 + N * N, A.begin());
    const bool ok_ll = tdls::TiledLuSolverStatic<double, N, ConfigLL>::solve<false, false, false>(
        A.data(), 1, piv.data(), 1, b0, x.data(), 1);
    TDLS_CHECK(ok_ll == solvable);

    std::copy(A0, A0 + N * N, A.begin());
    const bool ok_dyn = tdls::TiledLuSolverDynamic<double, ConfigRL>::solve(
...
TDLS_TEST_CASE("singular/zero-column-first") {
...
TDLS_TEST_CASE("singular/tiny-but-normal-matrix-is-solvable") {
