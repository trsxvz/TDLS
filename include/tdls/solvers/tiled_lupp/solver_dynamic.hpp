...
/// a fixed tile_size x tile_size footprint, while all loop bounds over
/// tiles and inside partial tiles are runtime values.
///
...
///
/// For equal shapes (same n, tile_size, schedule, TiledLUppConfig
/// thresholds), results are bitwise identical to the compile-time solver:
/// the arithmetic sequence is the same, only addressing and loop mechanics
/// differ.
///
...
///
/// Preconditions: n >= 1. tile_size may exceed n (the grid is then a single
...
/// \def TDLS_LUPP_DYN_BW
/// \brief Strided entry i of column w of a multi right-hand-side block of
/// b, addressed exactly as TDLS_LUPP_DYN_XW (b and x share both strides).
...
struct TiledLUppSolverDynamic {
    ...
    /// \brief Extent of the tile starting at row/column t0: tile_size, or
    /// less for the last tile.
    /// \param[in] t0 first global row/column of the tile
    /// \param[in] n  system dimension
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr int tile_size_at(const int t0,
                                                                        const int n) noexcept {
        return (n - t0 < tile_size) ? (n - t0) : tile_size;
    }

    /* =====================================================================
       Register-tile micro-kernels, runtime extents. A tile is a
       tile_size x tile_size array with row stride tile_size; the runtime
       extents bound the active part.
       ===================================================================== */
    ...
    ops_swap_rows(T* TDLS_RESTRICT t, const int k, const int r, const int ke) noexcept {
        for (int j = 0; j < ke; ++j) {
            ...
            t[k * tile_size + j] = t[r * tile_size + j];
            t[r * tile_size + j] = tmp;
        }
    }
    ...
    load_tile(const int n, const T* TDLS_RESTRICT A, const int A_stride,
        ...
        for (int i = 0; i < re; ++i) {
            for (int j = 0; j < ce; ++j)
                t[i * tile_size + j] = TDLS_LUPP_DYN_A(prow[i], col0 + j);
        }
    }
    ...
    store_tile(const int n, T* TDLS_RESTRICT A, const int A_stride, const int* TDLS_RESTRICT prow,
        ...
        for (int i = 0; i < re; ++i) {
            for (int j = 0; j < ce; ++j)
                TDLS_LUPP_DYN_A(prow[i], col0 + j) = t[i * tile_size + j];
        }
    }
    ...
    load_tile_piv(const int n, const T* TDLS_RESTRICT A, const int A_stride,
        ...
        for (int i = 0; i < re; ++i) {
            ...
            for (int j = 0; j < ce; ++j)
                t[i * tile_size + j] = TDLS_LUPP_DYN_A(phys, col0 + j);
        }
    }
    ...
    store_tile_piv(const int n, T* TDLS_RESTRICT A, const int A_stride,
        ...
        for (int i = 0; i < re; ++i) {
            const int phys = TDLS_LUPP_DYN_PIV(row0 + i);
            for (int j = 0; j < ce; ++j)
                TDLS_LUPP_DYN_A(phys, col0 + j) = t[i * tile_size + j];
        }
    }
    ...
    factor_diag_column(const int n, T* TDLS_RESTRICT A, const int A_stride, int* TDLS_RESTRICT piv,
        ...
        if (best >= oot_threshold) {
            ...
            for (int row = k0 + ke; row < n; ++row) {
                ...
                const T v = detail::abs(corrected);
                if (v > gbest) {
                    gbest     = v;
                    gbest_row = row;
                }
...

        {
            ...
            if (piv_row < k0 + ke) {
                ...
                for (int j = 0; j < ke; ++j) {
                    tile[c * tile_size + j] = TDLS_LUPP_DYN_A(phys, k0 + j);
                    if constexpr (schedule == Schedule::LeftLooking) {
                        for (int bj0 = 0; bj0 < k0; bj0 += tile_size)
                            for (int p = 0; p < tile_size; ++p)
                                tile[c * tile_size + j] -=
    ...
    factor_diag_tile(const int n, T* TDLS_RESTRICT A, const int A_stride, int* TDLS_RESTRICT piv,
        ...
        for (int c = 0; c < ke; ++c) {
            if (!factor_diag_column<oot_diagnostics, fuse_rhs>(
                    n, A, A_stride, piv, piv_stride, k0, tile, oot_count, c, ke, y, rhs_stride))
                return false;
        }
        return true;
    }
    ...
    rl_update_row_one(const int n, T* TDLS_RESTRICT A, const int A_stride,
        ...
        if constexpr (fuse_rhs) {
            for (int r = 0; r < ie; ++r) {
                T sum = T(0);
                for (int j = 0; j < ke; ++j)
                    sum += Aik[r * tile_size + j] * TDLS_LUPP_DYN_Y(k0 + j);
        ...
        for (int tj = k + 1; tj < nt; ++tj)
            rl_schur_one(n, A, A_stride, pk, pi, Aik, tj * tile_size, ke, ie,
                         tile_size_at(tj * tile_size, n));
    }
    ...
    rl_step(const int n, T* TDLS_RESTRICT A, const int A_stride, int* TDLS_RESTRICT piv,
        ...
        const int k0 = k * tile_size;
        const int nt = num_tiles(n);
        const int ke = tile_size_at(k0, n);

        T tile[tile_size * tile_size];
        load_tile_piv(n, A, A_stride, piv, piv_stride, k0, k0, tile, ke, ke);

        if (!factor_diag_tile<oot_diagnostics, fuse_rhs>(n, A, A_stride, piv, piv_stride, k0, tile,
        ...
        if constexpr (fuse_rhs) {
            for (int kk = 0; kk < ke; ++kk) {
                for (int i = kk + 1; i < ke; ++i)
                    TDLS_LUPP_DYN_Y(k0 + i) -= tile[i * tile_size + kk] * TDLS_LUPP_DYN_Y(k0 + kk);
            }
        }

        // TRSM right over the row panel
        for (int tj = k + 1; tj < nt; ++tj)
            rl_trsm_right_one(n, A, A_stride, pk, tile, tj * tile_size, ke,
                              tile_size_at(tj * tile_size, n));

        // TRSM down + Schur over the rows below
        for (int ti = k + 1; ti < nt; ++ti)
            rl_update_row_one<fuse_rhs>(n, A, A_stride, piv, piv_stride, pk, tile, k,
                                        ti * tile_size, ke, tile_size_at(ti * tile_size, n), y,
                                        rhs_stride);
        return true;
    }
    ...
    /// tiles bj < k0/tile_size of L(row0.., bj) * U(bj, col0..). All prior tiles
    /// are full.
    /// \param[in]     n          system dimension
    /// \param[in]     A          matrix (caller-pre-offset)
    /// \param[in]     A_stride   element stride of A
    ...
    ll_correct_tile(const int n, const T* TDLS_RESTRICT A, const int A_stride,
        ...
        for (int bj0 = 0; bj0 < k0; bj0 += tile_size) {
            ...
            load_tile_piv(n, A, A_stride, piv, piv_stride, row0, bj0, Lt, re, tile_size);
            T Ut[tile_size * tile_size];
            load_tile_piv(n, A, A_stride, piv, piv_stride, bj0, col0, Ut, tile_size, ce);
            ops_gemm_sub(t, Lt, Ut, re, ce, tile_size);
        }
    }
    ...
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void ll_update_below_one(
        ...
        // Fused forward substitution: B is the final L(i,k) panel, so push
        // the solved y_k segment into this row block.
        if constexpr (fuse_rhs) {
            for (int r = 0; r < ie; ++r) {
                T sum = T(0);
                for (int j = 0; j < ke; ++j)
                    sum += B[r * tile_size + j] * TDLS_LUPP_DYN_Y(k0 + j);
                TDLS_LUPP_DYN_Y(i0 + r) -= sum;
            }
        }
    }
    ...
    ll_step(const int n, T* TDLS_RESTRICT A, const int A_stride, int* TDLS_RESTRICT piv,
        ...
        for (int ti = k + 1; ti < nt; ++ti)
            ll_update_below_one<fuse_rhs>(n, A, A_stride, piv, piv_stride, tile, k0, ti * tile_size,
                                          ke, tile_size_at(ti * tile_size, n), y, rhs_stride);

        // U row panel right of the diagonal
        for (int tj = k + 1; tj < nt; ++tj)
            ll_update_right_one(n, A, A_stride, piv, piv_stride, tile, k0, tj * tile_size, ke,
                                tile_size_at(tj * tile_size, n));
        return true;
    }
    ...
    factorize(const int n, T* TDLS_RESTRICT A, const int A_stride, int* TDLS_RESTRICT piv,
        ...
        for (int k = 0; k < nt; ++k) {
            if constexpr (schedule == Schedule::RightLooking) {
                if (!rl_step<oot_diagnostics, fuse_rhs>(n, A, A_stride, piv, piv_stride, k,
                                                        oot_count, y, rhs_stride))
                    return false;
            } else {
                if (!ll_step<oot_diagnostics, fuse_rhs>(n, A, A_stride, piv, piv_stride, k,
                                                        oot_count, y, rhs_stride))
                    return false;
            }
        }
    ...
    /// for nrhs columns at once.
    /// \param[in]     n           system dimension
    /// \param[in]     nrhs        number of columns processed together
    ...
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    fwd_push_one(const int n, const int nrhs, const T* TDLS_RESTRICT A, const int A_stride,
                 ...
                 const int ke, const int me) noexcept {
        ...
        for (int i = 0; i < me; ++i) {
            for (int w = 0; w < nrhs; ++w) {
                T sum = T(0);
                for (int j = 0; j < ke; ++j)
                    sum += Lmk[i * tile_size + j] * TDLS_LUPP_DYN_XW(w, k0 + j);
    ...
    /// \param[in]     k           step index (k0 = k*tile_size)
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    fwd_step(const int n, const int nrhs, const T* TDLS_RESTRICT A, const int A_stride,
             const int* TDLS_RESTRICT piv, const int piv_stride, T* TDLS_RESTRICT x,
        ...
        for (int kk = 0; kk < ke; ++kk) {
            for (int i = kk + 1; i < ke; ++i) {
                for (int w = 0; w < nrhs; ++w)
                    TDLS_LUPP_DYN_XW(w, k0 + i) -=
                        Lkk[i * tile_size + kk] * TDLS_LUPP_DYN_XW(w, k0 + kk);
            }
        }

        // Push into the tiles below
        for (int m = k + 1; m < nt; ++m)
            fwd_push_one(n, nrhs, A, A_stride, piv, piv_stride, x, rhs_stride, xcol_stride, k0,
                         m * tile_size, ke, tile_size_at(m * tile_size, n));
    }
    ...
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    bwd_pull_one(const int n, const int nrhs, const T* TDLS_RESTRICT A, const int A_stride,
                 const int* TDLS_RESTRICT piv, const int piv_stride, T* TDLS_RESTRICT x,
                 const int rhs_stride, const int xcol_stride, const int k0, const int m0,
                 const int ke, const int me) noexcept {
        ...
        for (int i = 0; i < ke; ++i) {
            for (int w = 0; w < nrhs; ++w) {
                T sum = T(0);
                for (int j = 0; j < me; ++j)
                    sum += Ukm[i * tile_size + j] * TDLS_LUPP_DYN_XW(w, m0 + j);
    ...
    /// \param[in]     k           step index (k0 = k*tile_size)
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    bwd_step(const int n, const int nrhs, const T* TDLS_RESTRICT A, const int A_stride,
             const int* TDLS_RESTRICT piv, const int piv_stride, T* TDLS_RESTRICT x,
             const int rhs_stride, const int xcol_stride, const int k) noexcept {
        const int k0 = k * tile_size;
        const int nt = num_tiles(n);
        ...
        for (int m = k + 1; m < nt; ++m)
            bwd_pull_one(n, nrhs, A, A_stride, piv, piv_stride, x, rhs_stride, xcol_stride, k0,
                         m * tile_size, ke, tile_size_at(m * tile_size, n));
        ...
        for (int kk = ke - 1; kk >= 0; --kk) {
            for (int w = 0; w < nrhs; ++w)
                TDLS_LUPP_DYN_XW(w, k0 + kk) *= Ukk[kk * tile_size + kk]; // diag holds 1/pivot
            for (int i = 0; i < kk; ++i) {
                for (int w = 0; w < nrhs; ++w)
                    TDLS_LUPP_DYN_XW(w, k0 + i) -=
                        Ukk[i * tile_size + kk] * TDLS_LUPP_DYN_XW(w, k0 + kk);
            }
        }
    }
    ...
    /// \param[in]     xcol_stride element stride between columns of x
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    bwd_only(const int n, const int nrhs, const T* TDLS_RESTRICT A, const int A_stride,
             const int* TDLS_RESTRICT piv, const int piv_stride, T* TDLS_RESTRICT x,
             const int rhs_stride, const int xcol_stride) noexcept {
        for (int k = num_tiles(n) - 1; k >= 0; --k)
            bwd_step(n, nrhs, A, A_stride, piv, piv_stride, x, rhs_stride, xcol_stride, k);
    }

    /// \brief Triangular solves on already-permuted column(s) x. nrhs
    /// columns are processed per tile visit, so every L/U tile is loaded
    /// once for the whole block instead of once per column (nrhs = 1 = the
    /// single-RHS case).
    /// \param[in]     n           system dimension
    /// \param[in]     nrhs        number of columns processed together
    /// \param[in]     A           factored matrix (caller-pre-offset)
    ...
    /// \param[in]     xcol_stride element stride between columns of x
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    fwd_bwd(const int n, const int nrhs, const T* TDLS_RESTRICT A, const int A_stride,
            const int* TDLS_RESTRICT piv, const int piv_stride, T* TDLS_RESTRICT x,
            const int rhs_stride, const int xcol_stride) noexcept {
        const int nt = num_tiles(n);
        for (int k = 0; k < nt; ++k)
            fwd_step(n, nrhs, A, A_stride, piv, piv_stride, x, rhs_stride, xcol_stride, k);

        bwd_only(n, nrhs, A, A_stride, piv, piv_stride, x, rhs_stride, xcol_stride);
    }

    /// \brief In-place application of the permutation to one strided
    /// column x, by cycle decomposition: the permute half of
    /// substitute_inplace, shared with substitute_inplace_multirhs (which
    /// applies it to each of its columns).
    ///
    /// Each cycle is rotated once from its smallest index. Up to n = 64 a
    /// 64-bit bitmask tracks the visited rows; beyond, a cycle-leader scan
    /// is used instead (zero storage, an extra integer walk of each
    /// orbit). The dispatch is warp-uniform on GPU (n is the same for
    /// every lane). Both paths move the same values in the same order.
    /// \param[in]     n          system dimension
    /// \param[in]     piv        permutation from factorize
    /// \param[in]     piv_stride element stride of piv
    /// \param[in,out] x          column to permute
    /// \param[in]     rhs_stride element stride of x
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    permute_inplace(const int n, const int* TDLS_RESTRICT piv, const int piv_stride,
    ...
    substitute(const int n, const T* TDLS_RESTRICT A, const int A_stride,
        ...
        fwd_bwd(n, 1, A, A_stride, piv, piv_stride, x, rhs_stride, 0);
    }
    ...
    /// triangular sweep for all nrhs columns together. Internal engine
    /// of substitute_multirhs.
    /// \param[in]  n           system dimension
    /// \param[in]  nrhs        number of columns of this pass
    /// \param[in]  A           factored matrix produced by factorize
    /// \param[in]  A_stride    element stride of A
    /// \param[in]  piv         permutation produced by factorize
    /// \param[in]  piv_stride  element stride of piv
    /// \param[in]  b           nrhs right-hand-side columns, in original
    ///             (unpermuted) order
    /// \param[out] x           nrhs solution columns
    /// \param[in]  rhs_stride  element stride of b and x
    /// \param[in]  xcol_stride element stride between columns of b and x
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    substitute_multirhs_pass(const int n, const int nrhs, const T* TDLS_RESTRICT A,
                             const int A_stride, const int* TDLS_RESTRICT piv, const int piv_stride,
                             const T* TDLS_RESTRICT b, T* TDLS_RESTRICT x, const int rhs_stride,
                             const int xcol_stride) noexcept {
        for (int i = 0; i < n; ++i) {
            const int p = TDLS_LUPP_DYN_PIV(i);
            for (int w = 0; w < nrhs; ++w)
                TDLS_LUPP_DYN_XW(w, i) = TDLS_LUPP_DYN_BW(w, p);
        }
        fwd_bwd(n, nrhs, A, A_stride, piv, piv_stride, x, rhs_stride, xcol_stride);
    }

    /// \brief Solve nrhs right-hand-side columns from a prior factorize:
    /// X := U^-1 L^-1 P B, column by column. b and x must not alias (use
    /// substitute_inplace_multirhs for the aliased single-buffer
    /// pattern).
    ///
    /// Column w lives at b/x + w*xcol_stride; b and x share both strides,
    /// as they share rhs_stride in substitute. The columns are solved in
    /// passes of pass_width columns; within a pass, every L/U tile is
    /// loaded once for all the columns of the pass. Per-column
    /// arithmetic is identical whatever the cutting, and identical to
    ...
    ///
    /// The permutation is applied in place by cycle decomposition (see
    /// permute_inplace).
    /// \param[in]     n          system dimension
    /// \param[in]     A          factored matrix produced by factorize
    /// \param[in]     A_stride   element stride of A
    ...
    substitute_inplace(const int n, const T* TDLS_RESTRICT A, const int A_stride,
                       const int* TDLS_RESTRICT piv, const int piv_stride, T* TDLS_RESTRICT x,
                       const int rhs_stride) noexcept {
        permute_inplace(n, piv, piv_stride, x, rhs_stride);
        fwd_bwd(n, 1, A, A_stride, piv, piv_stride, x, rhs_stride, 0);
    }

    /// \brief One pass of the in-place multi right-hand-side
    /// substitution: nrhs columns of x permuted by the cycle
    /// decomposition of permute_inplace, then one triangular sweep for
    /// all nrhs columns together. Internal engine of
    ...
    /// \param[in]     piv         permutation produced by factorize
    /// \param[in]     piv_stride  element stride of piv
    /// \param[in,out] x           nrhs right-hand-side columns on entry,
    ///                nrhs solution columns on exit
    /// \param[in]     rhs_stride  element stride of x
    /// \param[in]     xcol_stride element stride between columns of x
    TDLS_HOST_DEVICE TDLS_FORCEINLINE static constexpr void
    substitute_inplace_multirhs_pass(const int n, const int nrhs, const T* TDLS_RESTRICT A,
                                     const int A_stride, const int* TDLS_RESTRICT piv,
                                     const int piv_stride, T* TDLS_RESTRICT x, const int rhs_stride,
                                     const int xcol_stride) noexcept {
        for (int w = 0; w < nrhs; ++w)
            permute_inplace(n, piv, piv_stride, x + unsigned(w) * unsigned(xcol_stride),
                            rhs_stride);
        fwd_bwd(n, nrhs, A, A_stride, piv, piv_stride, x, rhs_stride, xcol_stride);
    }

    /// \brief nrhs columns solved in place: x holds the nrhs unpermuted
    /// right-hand-side columns on entry and the nrhs solutions on exit.
    ///
    /// The permutation is applied to each column by the cycle
    /// decomposition of permute_inplace. The columns are solved in
    /// passes of pass_width columns; within a pass, every L/U tile is
    /// loaded once for all the columns of the pass. Per-column values
    ...
    ///                further substitute* calls)
    /// \param[in]     A_stride    element stride of A
    /// \param[out]    piv         permutation storage (always
    ///                caller-provided)
    /// \param[in]     piv_stride  element stride of piv
    /// \param[in]     b           nrhs right-hand-side columns, in
    ///                original order
    /// \param[out]    x           nrhs solution columns
    /// \param[in]     rhs_stride  element stride of b and x
    /// \param[in]     xcol_stride element stride between columns of b and x
    ...
    /// factorization, so the separate forward pass disappears, and all
    /// its tile reloads with it; only the backward pass remains. y follows the
    ...
    solve_inplace(const int n, T* TDLS_RESTRICT A, const int A_stride, int* TDLS_RESTRICT piv,
                  ...
                  int& oot_count) noexcept {
        if (!factorize<oot_diagnostics, true>(n, A, A_stride, piv, piv_stride, oot_count, y,
                                              rhs_stride))
            return false;

        // Backward pass only: the forward one happened inside factorize.
        bwd_only(n, 1, A, A_stride, piv, piv_stride, y, rhs_stride, 0);
        return true;
    }
    ...
    /// \brief factorize + substitute_inplace_multirhs in one call: y holds
    /// the nrhs unpermuted right-hand-side columns on entry and the nrhs
    /// solutions on exit.
    ///
    ...
    ///                caller-provided)
    /// \param[in]     piv_stride  element stride of piv
    /// \param[in,out] y           nrhs right-hand-side columns on entry,
    ///                nrhs solution columns on exit
    /// \param[in]     rhs_stride  element stride of y
